...
def num_digits_neg_str_mix(v):
    if isinstance(v, int):
        return num_digits(v if v >= 0 else -1*v)
...
def check_int_str(stringa, intero):
    return len(stringa.split(str(intero))) - 1
...
def count_sub_string(query, corpo):
    shift = len(query)
    res = 0
    for i in range(corpo.__len__()-shift+1):
        if corpo[i:i+shift] == query:
            res += 1
    return res
...
def check_reverse(S, T):
    return S == ''.join([x for x in reversed(T)])
...
def get_list_except_min_max(L):
    ...
    for i, el in enumerate(L):
        ...
        if el < _min:
            _min = el
            min_pos = i
    return L.pop(min_pos), L.pop(max_pos - 1 if min_pos < max_pos else max_pos)
...
def get_list_except_min_max_general(L):
    ...
    res = L.count(_min)
    while L.count(_min) > 0:
        L.remove(_min)
    for i, el in enumerate(L):
        if el < _max:
            _max = el
    res += L.count(_max)
    while L.count(_max) > 0:
        L.remove(_max)
    return res
...
def sort_by_str(T):
    res = list(T)
    _sort1 = lambda x: len(x)
    _sort2 = lambda x: -1*ord(x[0])
    res.sort(key=_sort2)
    res.sort(key=_sort1)
    return tuple(res)
...
def sort_by_month(L):
    ...
    _sort = lambda x: mesi.index(x.lower())
    L.sort(key=_sort)
    return L
...
def int_to_hist(values):
    res = ''
    els = range(min(values), max(values)+1)
    for el in els:
        res += f'{el}\t{"*"*values.count(el)}\n'
    return res
