...
def max_recursive(l: List[int]) -> int:
    if len(l) == 1:
        return l[0]
    res = max_recursive(l[1:])
    return l[0] if l[0] >= res else res
...
def is_prime_recursive(n: int, d=2) -> bool:
    ...
    if n == d:
        return True
    return is_prime_recursive(n, d+1) and not n % d == 0
...
def knapsack(l: List[Tuple[int, int]], W: int) -> Tuple[int, int]:
    if len(l) == 1:
        return l[0] if l[0][0] <= W else (0, l[0][1])
    a = knapsack(l[1:], W)
    return (l[0][0] + a[0], l[0][1] + a[1]) if l[0][0] + a[0] <= W else a
